...
and Windows. Choose a bright warm voice or a deep bright voice in Korean,
Japanese, or English.
...
Codex deliberately does not trust third-party hooks at install time.
...
Defaults are `female`, `ko`, all ten events enabled, and a 450 ms minimum interval.
`PermissionRequest` only plays when Codex actually asks for permission.

## Compatibility

Version 0.1.0 supports macOS and Windows with only system-provided audio and
